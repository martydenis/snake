import { throttle, dpi, getDistance, getAngleBetweenPoints } from './sandbox-utils/utilities.js';
...
function Joystick(options) {
    ...
    self._init = function () {
        self.cfg = { ...self._settings, ...options };

        if (self.cfg.static) {
            self._hidden = false;
        }

        self._pos = { ...self.cfg.initialPosition };
        self._stickPos = { ...self._pos };

        // Event listeners
        window.addEventListener('touchstart', self.event._onTouchstart);
        window.addEventListener('touchend', self.event._onTouchend);
        if (self.cfg.adaptivePositioning)
            window.addEventListener('touchmove', throttle(self.event._onTouchmove, 50));
        else
            window.addEventListener('touchmove', self.event._onTouchmove);
    }

    self._getDirection = function () {
        if (self._distance < self.cfg.deadzone * self.cfg.baseRadius) {
            return false;
        }
    ...
    self.event = {
        _onTouchstart: function (e) {
            ...
            if (self.cfg.static) {
                self._distance = getDistance(self._pos.x, self._pos.y, x, y);
                if (self._distance > self.cfg.baseRadius) {
                    return;
                }
            } else {
                self._hidden = false;
        ...
        _onTouchmove: function (e) {
            if (self._disabled || !self._holding) return;

            const x = e.targetTouches[0].pageX;
            const y = e.targetTouches[0].pageY;
            const currentDirection = self._getDirection();

            self.moveStick(x, y);

            if (currentDirection != self._direction) {
                if (currentDirection === false) {
                    self.cfg.onStop();
                } else {
                    self.cfg.onMove(currentDirection, self._distance);
                }
            }
            ...
            self._direction = currentDirection;
        },
        _onTouchend: function (e) {
            ...
            self._holding = false;
            self.moveStick(self._pos.x, self._pos.y);
            self.cfg.onStop();
        },
    }
    ...
    self.moveStick = function (x, y) {
        self._distance = Math.min(self.cfg.maxDistance, getDistance(self._pos.x, self._pos.y, x, y));
        self._angle = getAngleBetweenPoints(self._pos, {x, y});

        self._stickPos.x = self._pos.x + Math.cos(self._angle) * self._distance;
        self._stickPos.y = self._pos.y + Math.sin(self._angle) * self._distance;
    ...
    self.draw = function () {
        ...
        self.cfg.context.arc(dpi(self._pos.x), dpi(self._pos.y), dpi(self.cfg.baseRadius), 0, 2 * Math.PI);
        ...
        self.cfg.context.beginPath();
        self.cfg.context.arc(dpi(self._stickPos.x), dpi(self._stickPos.y), dpi(self.cfg.stickRadius), 0, 2 * Math.PI);
